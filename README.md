...
* makeを用いることで、修正していない`cpp`ファイルのビルドが除外されるので、2回からビルド時間が大幅に短縮されます。
...
1. **ビルドと実行:**
    * **実行 (Run):** Makefile Tools パネルの上部にある **再生ボタン（右三角 ▶）** をクリックします。自動的に `mingw32-make` が呼び出されてコンパイル・ビルドが実行され、完了後に `vscodeLIA.exe` が起動します。１回目のビルドは数十秒かかります。ImGUI関係のソースファイルが多いのが原因の一つです。最適化オプション (-O2) を入れるとビルド時間がさらに数倍長くなります。並列ビルドをすると早くなりそうですがVS Codeでのやり方がわかりません。
